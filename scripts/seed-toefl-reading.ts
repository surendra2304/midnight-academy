...
export async function seedToeflReadingBank() {
  ...
  await supabase.from('sections').upsert(
    {
      ...
      section_order: 0,
      timing_seconds: 1800,
      instructions: 'The Reading section measures your ability to understand academic and practical English texts.',
    },
    { onConflict: 'id' },
  );
  ...
  await supabase.from('content_items').upsert({
    ...
    payload: {
      passage: 'Urban gardens provide [0] fresh produce and foster community cooperation. Residents can [1] their own vegetables while learning sustainable agricultural practices.',
      blanks: [
  ...
  await supabase.from('content_items').upsert({
    ...
    payload: {
      ...
      passage: 'Starting October 1st, the campus shuttle will operate every 15 minutes during peak hours (7:30 AM - 10:00 AM) and every 30 minutes during off-peak times. Students must tap their active university ID cards upon boarding. Weekend service remains suspended until further notice.',
  ...
  await supabase.from('question_options').upsert([
    {
      ...
      is_correct: false,
      distractor_rationale: 'Corals provide these byproducts; their depletion is not cited as the expulsion trigger.',
      option_order: 0,
    },
    {
      content_item_id: item3Id,
