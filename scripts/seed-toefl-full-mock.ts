...
export async function seedFullToeflMock() {
  ...
  await supabase.from('modules').upsert([
    { id: modRId, section_id: secReadingId, stage_index: 1, difficulty_band: 'middle', module_order: 0 },
    { id: modLId, section_id: secListeningId, stage_index: 1, difficulty_band: 'middle', module_order: 0 },
    { id: modWId, section_id: secWritingId, stage_index: 1, difficulty_band: 'middle', module_order: 0 },
    { id: modSId, section_id: secSpeakingId, stage_index: 1, difficulty_band: 'middle', module_order: 0 },
  ]);
  ...
  await supabase.from('content_items').upsert({
    ...
    payload: {
      ...
      passage: 'Plants convert sunlight into chemical energy through light-dependent reactions followed by the Calvin cycle.',
  ...
  await supabase.from('question_options').upsert([
    { content_item_id: mockItemR, option_key: 'A', option_text: 'Fix atmospheric carbon into glucose', is_correct: true, option_order: 0 },
    { content_item_id: mockItemR, option_key: 'B', option_text: 'Directly split water molecules', is_correct: false, distractor_rationale: 'Water splitting occurs in light-dependent reactions.', option_order: 1 },
  ]);
  ...
  await supabase.from('question_options').upsert([
    { content_item_id: mockItemL, option_key: 'A', option_text: 'Thermohaline density differences driven by temperature and salinity', is_correct: true, option_order: 0 },
    { content_item_id: mockItemL, option_key: 'B', option_text: 'Lunar gravitational tides only', is_correct: false, distractor_rationale: 'Tides cause local oscillations, not global conveyor circulation.', option_order: 1 },
  ]);
  ...
  await supabase.from('content_items').upsert({
    ...
    payload: {
      ...
      context: 'You are requesting a room change due to severe noise disruptions during your study hours.',
      prompt: 'Write an email politely explaining the noise issue and requesting a transfer to a quiet-study floor.',
      modelAnswer: 'Dear Director of Student Housing,\n\nI am writing to respectfully request a room reassignment...',
    },
    item_order: 0,
  });
  ...
  await supabase.from('content_items').upsert({
    ...
    payload: {
      prompt: 'Do you prefer studying alone or in study groups? Explain your reasons with specific examples.',
      preparationSeconds: 15,
      responseLimitSeconds: 45,
      modelAnswer: 'I strongly prefer studying in groups because collaborative problem solving improves retention...',
    },
    item_order: 0,
  });
