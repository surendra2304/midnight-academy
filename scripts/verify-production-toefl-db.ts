...
const supabaseAnonKey = process.env.VITE_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY || supabaseSecretKey;
...
export async function verifyProductionToeflDb() {
  ...
  if (pErr) {
    ...
    for (const v of publishedVersions || []) {
      ...
      console.log(`  - [${t?.code || 'NO-CODE'}] ${t?.name} (${t?.category}) -> Version ID: ${v.id}`);
    }
  }
  ...
  for (const version of publishedVersions || []) {
    const { data: sections } = await supabaseAdmin
      .from('sections')
      .select('id, section_type, timing_seconds, modules(id, content_items(id, item_type, section_type, question_options(id, option_key, option_text, is_correct)))')
    ...
    if (!valResult.isValid) {
      ...
      console.log(`  [VALID] Blueprint ${(version.tests as { name?: string })?.name} passed all sanity checks.`);
    }
  }
  ...
  console.log(`  Anon query result for question_options:`, anonOptions ? `${anonOptions.length} rows returned` : anonErr?.message);

  // 5. Verification of Legacy Data Preservation
  console.log('\n5. Verifying Legacy Technical Comprehension Data Integrity...');
  const { count: testsCount } = await supabaseAdmin.from('tests').select('*', { count: 'exact', head: true });
  const { count: questionsCount } = await supabaseAdmin.from('questions').select('*', { count: 'exact', head: true });
  const { count: attemptsCount } = await supabaseAdmin.from('attempts').select('*', { count: 'exact', head: true });
