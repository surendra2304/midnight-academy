...
async function test() {
  ...
  const gibberishRes = await evaluateAnswer({
    questionText: "The library allows members to borrow up to five books at a time for a period of two weeks.",
  ...
  const correctRes = await evaluateAnswer({
    ...
    concepts: ["Borrowing Limit", "Loan Duration"],
    constraints: ["Max 5 books", "2 weeks return window"],
    response: "You can take at most 5 books from the library and you must return them within 2 weeks."
  });
