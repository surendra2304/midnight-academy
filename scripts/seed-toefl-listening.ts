...
export async function seedToeflListeningBank() {
  ...
  await supabase.from('sections').upsert(
    {
      ...
      section_order: 0,
      timing_seconds: 1740,
      instructions: 'The Listening section measures your ability to understand spoken English in academic and campus contexts.',
    },
    { onConflict: 'id' },
  );
  ...
  await supabase.from('content_assets').upsert({
    ...
    duration_ms: 12000,
    metadata: {
      transcript: 'Speaker A: Excuse me, professor! Are there still open spots for the Tuesday evening chemistry lab, or has registration already closed for the term?',
    },
  });
  ...
  await supabase.from('question_options').upsert([
    ...
    },
    {
      ...
      is_correct: false,
      distractor_rationale: 'The speaker is asking about registration availability, not an assignment extension.',
      option_order: 1,
    },
    {
      content_item_id: item1Id,
  ...
  await supabase.from('content_items').upsert({
    ...
    payload: {
      ...
      prompt: 'According to the lecture, what is one major limitation of enhanced geothermal systems (EGS)?',
    },
    item_order: 1,
  });
  ...
  await supabase.from('content_assets').upsert({
    ...
    duration_ms: 28000,
    metadata: {
      transcript: 'Professor: While conventional geothermal plants require natural underground aquifers, Enhanced Geothermal Systems—or EGS—inject pressurized fluid to fracture impermeable hot rock. While this vastly expands suitable geographic locations, the high pressures involved can occasionally induce minor micro-seismic activity, which remains a primary engineering and public concern.',
    },
  });
  ...
  await supabase.from('question_options').upsert([
    {
      ...
      is_correct: false,
      distractor_rationale: 'EGS actually expands locations beyond natural hydrothermal reservoirs.',
      option_order: 0,
    },
    {
      content_item_id: item2Id,
