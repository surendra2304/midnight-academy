...
export async function seedToeflSpeakingBank() {
  ...
  await supabase.from('sections').upsert(
    {
      ...
      section_order: 0,
      timing_seconds: 480,
      instructions: 'The Speaking section measures your ability to speak English effectively in academic and interview settings.',
    },
    { onConflict: 'id' },
  );
  ...
  await supabase.from('rubrics').upsert({
    ...
    traits: [
      { name: 'task_fulfillment', weight: 0.25, description: 'Directly addresses prompt with sufficient elaboration', maxScore: 6.0 },
      { name: 'organization', weight: 0.20, description: 'Coherent progression of ideas and clear transitions', maxScore: 6.0 },
      { name: 'language_use', weight: 0.20, description: 'Grammar control, syntactic complexity, vocabulary precision', maxScore: 6.0 },
      { name: 'delivery', weight: 0.20, description: 'Pacing, fluency, minimal unnatural pauses', maxScore: 6.0 },
      { name: 'pronunciation', weight: 0.15, description: 'Phonetic intelligibility and natural intonation', maxScore: 6.0 },
    ],
    band_descriptors: {
      '6.0': 'Highly intelligible speech with natural pacing, sophisticated vocabulary, and well-developed supporting arguments.',
      '5.0': 'Clear and fluent response with minor pauses or pronunciation lapses that do not hinder comprehension.',
      '4.0': 'Generally understandable speech with noticeable hesitation and basic grammar structures.',
  ...
  await supabase.from('content_items').upsert({
    ...
    payload: {
      ...
      targetSentence: 'Renewable energy projects require significant initial capital investment but yield long-term environmental dividends.',
      modelAnswer: 'Renewable energy projects require significant initial capital investment but yield long-term environmental dividends.',
    },
    item_order: 0,
  });
