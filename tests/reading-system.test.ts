...
describe('TOEFL Reading System & Scoring Suite', () => {
  describe('Deterministic Reading Scoring', () => {
    const mcqRule = {
      ...
      difficulty: 'Easy',
      options: [
        { optionKey: 'A', optionText: 'Pay cash', isCorrect: false, distractorRationale: 'No cash needed.' },
        { optionKey: 'B', optionText: 'Tap active ID card', isCorrect: true },
        { optionKey: 'C', optionText: 'Reserve 24h ahead', isCorrect: false, distractorRationale: 'No reservation required.' },
      ],
    };
    ...
    it('scores multi-blank Complete the Words with partial credit', () => {
      ...
      // Full credit
      const fullRes = readingScoringService.scoreItem(JSON.stringify(['abundant', 'grow']), clozeRule);
      ...
      const partialRes = readingScoringService.scoreItem(JSON.stringify(['abundant', 'wrong_word']), clozeRule);
      ...
      const altRes = readingScoringService.scoreItem(JSON.stringify(['PLENTIFUL', 'Cultivate']), clozeRule);
