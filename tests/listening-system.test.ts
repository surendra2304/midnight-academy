...
describe("TOEFL Listening System & Audio Pipeline Suite", () => {
  describe("Audio Asset Health & Mime Validation", () => {
    it("validates supported audio MIME types correctly", () => {
      expect(
        audioAssetService.validateAssetHealth({
          storage_path: "tracks/audio.mp3",
          mime_type: "audio/mpeg",
        }),
      ).toBe(true);
      expect(
        audioAssetService.validateAssetHealth({
          storage_path: "tracks/audio.ogg",
          mime_type: "audio/ogg",
        }),
      ).toBe(true);
      expect(
        audioAssetService.validateAssetHealth({
          storage_path: "tracks/audio.wav",
          mime_type: "audio/wav",
        }),
      ).toBe(true);
      expect(
        audioAssetService.validateAssetHealth({
          storage_path: "tracks/video.mp4",
          mime_type: "video/mp4",
        }),
      ).toBe(false);
      expect(
        audioAssetService.validateAssetHealth({ storage_path: "", mime_type: "audio/mpeg" }),
      ).toBe(false);
    });
  });
  ...
  describe("Deterministic Scoring for Listening Items", () => {
    const listeningRule = {
      ...
      options: [
        {
          optionKey: "A",
          optionText: "Volcanic vents only",
          isCorrect: false,
          distractorRationale: "EGS expands to non-volcanic regions.",
        },
        {
          optionKey: "B",
          optionText: "High pressure can induce micro-seismic activity",
          isCorrect: true,
        },
        {
          optionKey: "C",
          optionText: "Large CO2 emissions",
          isCorrect: false,
          distractorRationale: "Geothermal systems emit minimal CO2.",
        },
      ],
    };
