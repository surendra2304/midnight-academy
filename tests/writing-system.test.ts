...
describe("TOEFL Writing & Gemini Evaluation Engine Suite", () => {
  ...
  describe("Trait to Band Conversion", () => {
    it("converts trait scores to exact 0.5 half-point increments accurately", () => {
      expect(
        traitsToBand({ task_fulfillment: 6.0, organization: 6.0, language_use: 6.0 }).scoreBand,
      ).toBe(6.0);
      expect(
        traitsToBand({ task_fulfillment: 5.0, organization: 5.5, language_use: 5.0 }).scoreBand,
      ).toBe(5.0); // avg 5.16 -> 5.0
      expect(
        traitsToBand({ task_fulfillment: 5.5, organization: 5.5, language_use: 5.0 }).scoreBand,
      ).toBe(5.5); // avg 5.33 -> 5.5
      expect(
        traitsToBand({ task_fulfillment: 1.0, organization: 1.0, language_use: 1.0 }).scoreBand,
      ).toBe(1.0);
    });
  });
