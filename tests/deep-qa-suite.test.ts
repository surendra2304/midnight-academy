...
import { calculateRemainingSeconds, sessionReducer, type SessionSnapshot, type ClientTestBlueprint } from '../src/lib/tests/session-state';
...
describe('Deep QA & Error-Resistance Regression Suite', () => {
  ...
  it('protects against race condition transitions on finished sessions by throwing state guard error', () => {
    ...
    expect(() => {
      sessionReducer(finalizedState, { type: 'ADVANCE_SECTION', nextSectionIndex: 1, sectionDurationSeconds: 1800 }, dummyBlueprint);
  ...
  it('handles negative or overflow ratings safely in SuperMemo SM-2', () => {
    const schedule = calculateNextSRSState({ repetitions: 0, intervalDays: 1, easeFactor: 2.5 }, 'again');
