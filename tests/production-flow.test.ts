...
describe("Production Flow Verification (No Mock-Data Dependencies in Live Flows)", () => {
  it("verifies /test.run.tsx uses FullMockRunnerOrchestrator and engine RPCs", () => {
    const content = readFileSync(resolve(__dirname, "../src/routes/test.run.tsx"), "utf-8");
    expect(content).toContain("resumeToeflAttempt");
    expect(content).toContain("FullMockRunnerOrchestrator");
  });
  ...
  it("verifies /result.$attemptId.tsx uses getToeflScoreReport and UnifiedScoreReportView", () => {
    const content = readFileSync(
      resolve(__dirname, "../src/routes/result.$attemptId.tsx"),
      "utf-8",
    );
    expect(content).toContain("getToeflScoreReport");
    expect(content).toContain("UnifiedScoreReportView");
  });

  it("verifies /dashboard.tsx loads from getStudentAnalyticsDashboard RPC", () => {
    const content = readFileSync(resolve(__dirname, "../src/routes/dashboard.tsx"), "utf-8");
    expect(content).toContain("getStudentAnalyticsDashboard");
    expect(content).toContain("getStudentPracticeQueue");
  });
