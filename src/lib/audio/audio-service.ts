...
export class AudioAssetService {
  ...
  async resolveItemAudio(contentItemId: string): Promise<AudioAssetMetadata | null> {
    const { data: asset, error } = await supabaseAdmin
      .from("content_assets")
      .select(
        "id, content_item_id, asset_type, storage_path, mime_type, duration_ms, checksum, metadata",
      )
    ...
    if (!asset.storage_path.startsWith("http://") && !asset.storage_path.startsWith("https://")) {
      const { data } = supabaseAdmin.storage.from("toefl_audio").getPublicUrl(asset.storage_path);
