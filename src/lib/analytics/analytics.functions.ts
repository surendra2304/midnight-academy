...
export const getStudentAnalyticsDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: scoreReports } = await supabaseAdmin
      .from("score_reports")
      .select(
        "attempt_id, student_id, overall_band, reading_band, listening_band, writing_band, speaking_band, comparable_score, generated_at",
      )
    ...
    const { data: responses } = await supabaseAdmin
      .from("responses")
      .select(
        "id, attempt_section_id, content_item_id, is_correct, score, time_spent_ms, answered_at, content_items(id, section_type, item_type, difficulty, skill_tags), attempt_sections(attempt_id)",
      )
      .eq("student_id", studentId);
    ...
    const rawMetrics: RawAttemptMetricInput[] = (responses || []).map((r) => {
      const item = (
        r as unknown as {
          ...
          attempt_sections: { attempt_id: string };
        }
      ).content_items;
      const attId =
        (r as unknown as { attempt_sections: { attempt_id: string } }).attempt_sections
          ?.attempt_id || "";
    ...
    const latestReport = (scoreReports || [])[0];

    return {
      profile,
      targetBand: (latestReport as any)?.target_score || 5.0,
    };
  });
...
export const explainWeaknessProfileAi = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((data) =>
    z.object({ weakSkills: z.array(z.string()), errorPatterns: z.array(z.string()) }).parse(data),
  )
  .handler(async ({ data }) => {
    try {
      ...
      return {
        explanation:
          "Focus your practice on strengthening your lowest-accuracy skills and reviewing distractor rationales.",
      };
    }
  });
