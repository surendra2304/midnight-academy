...
export const generateAiQuestionDraft = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!userRole || (userRole.role !== 'admin' && userRole.role !== 'instructor')) {
...
Output valid JSON strictly following this schema:
{
  ...
  "options": [
    {
      ...
      "optionText": "<Text>",
      "isCorrect": true,
      "distractorRationale": "<Why this is correct or distractor rationale>"
    },
    {
      "optionKey": "B",
      "optionText": "<Text>",
      "isCorrect": false,
      "distractorRationale": "<Specific reason why this distractor is incorrect>"
    },
    {
      "optionKey": "C",
      "optionText": "<Text>",
  ...
  "modelAnswer": "<Exemplary reference answer scoring 6.0 band>"
}`;

    const userMessage = `TASK TYPE: ${data.itemType}\nSECTION: ${data.sectionType}\nTOPIC: ${data.topic}\nDIFFICULTY: ${data.difficulty}\nSKILL TAGS: ${(data.skillTags || []).join(', ')}`;
...
// 2. Pre-Publish Validation Check
export const validateTestVersion = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    // Fetch test version and hydrated sections
    const { data: version } = await supabaseAdmin
      .from('test_versions')
      .select('id, test_id, tests(name)')
      .eq('id', data.testVersionId)
      .single();

    if (!version) throw new Error('Test version not found');
