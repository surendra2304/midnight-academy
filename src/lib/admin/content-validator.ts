...
export class ContentValidator {
  ...
  validateBlueprint(blueprint: ValidationBlueprintSpec): ValidationResult {
    ...
    if (!blueprint.sections || blueprint.sections.length === 0) {
      errors.push({
        path: "blueprint.sections",
        message: "Test must contain at least one section.",
        severity: "error",
      });
    ...
    for (let sIdx = 0; sIdx < blueprint.sections.length; sIdx++) {
      ...
      if (sec.timingSeconds <= 0) {
        errors.push({
          path: `${secPath}.timingSeconds`,
          message: "Section timing must be greater than 0 seconds.",
          severity: "error",
        });
      }
      ...
      if (!sec.items || sec.items.length === 0) {
        errors.push({
          path: `${secPath}.items`,
          message: `Section '${sec.sectionType}' has no content items attached.`,
          severity: "error",
        });
        continue;
      }
      ...
      for (let iIdx = 0; iIdx < sec.items.length; iIdx++) {
        ...
        if (
          ...
          if (!item.options || item.options.length < 2) {
            errors.push({
              path: itemPath,
              message: "Multiple choice item must have at least 2 options.",
              severity: "error",
            });
          } else {
            ...
            if (correctCount === 0) {
              errors.push({
                path: itemPath,
                message: "Question is missing a correct answer key (isCorrect = true).",
                severity: "error",
              });
            } else if (correctCount > 1) {
              warnings.push({
                path: itemPath,
                message: "Question has multiple correct answer keys flagged.",
                severity: "warning",
              });
            }
          }
        ...
        if (item.itemType === "build_sentence") {
          if (!item.acceptedSequences || item.acceptedSequences.length === 0) {
            errors.push({
              path: itemPath,
              message: "Build a sentence item must have at least one accepted sequence key.",
              severity: "error",
            });
          }
        }
        ...
        if (item.itemType === "dictation") {
          if (!item.promptSnippet || item.promptSnippet.trim().length === 0) {
            errors.push({
              path: itemPath,
              message: "Dictation item must contain a reference sentence transcript.",
              severity: "error",
            });
          }
        }
        ...
        if (
          ...
          if (!item.audioAssetPath || item.audioAssetPath.trim().length === 0) {
            warnings.push({
              path: itemPath,
              message: "Listening item does not reference an audio asset track.",
              severity: "warning",
            });
          }
        }
