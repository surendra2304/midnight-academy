  ...
  if (!currentItem) {
    return (
      <div className="rounded-2xl border border-border bg-card/60 p-8 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">You have practiced all oral shadowing sentences in this set.</p>
      ...
      <div className="rounded-3xl border border-border bg-card/60 p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">{currentItem.contextNote || 'Listen and shadow verbatim.'}</p>
          </div>
        ...
        <div className="space-y-2">
          <span className="text-[10px] font-bold uppercase tracking-wider text-primary">1. Model Speaker Audio</span>
        ...
        <div className="space-y-2">
          <span className="text-[10px] font-bold uppercase tracking-wider text-primary">2. Your Shadowing Speech</span>
          ...
          <div className="flex flex-col items-center justify-center p-6 bg-gradient-to-b from-surface-2/20 to-surface-2/5 rounded-2xl border border-border/60 space-y-4">
            ...
            <div className="text-center space-y-1">
              <p className="text-xs font-bold text-foreground">
                {isRecording ? `Recording... (${recordSeconds}s) Click to Stop & Score` : 'Click Microphone to Repeat Sentence'}
              </p>
              {speechTranscript ? (
        ...
        {isEvaluating ? (
          <div className="flex items-center justify-center gap-3 p-8 bg-card/30 rounded-2xl border border-border/80">
            ...
            <p className="text-sm font-semibold text-muted-foreground">Evaluating speech rhythm, pronunciation & accuracy...</p>
          </div>
        ) : result ? (
          <div className="space-y-5 rounded-2xl border border-border bg-card/50 p-6">
            ...
            <div className="flex items-center justify-between border-b border-border pb-4">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-primary">Shadowing Evaluation</span>
                <h4 className="text-xl font-black text-foreground">
                  Score: Band <span className="text-primary">{result.scoreBand.toFixed(1)}</span> / 6.0
                </h4>
              </div>

              <div className="text-right">
                <span className="text-[10px] uppercase font-bold text-muted-foreground">Repetition Accuracy</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3 rounded-xl bg-surface-2/40 border border-border/60 text-center">
                <p className="text-[10px] font-bold uppercase text-muted-foreground">Pronunciation</p>
                <p className="text-base font-black text-primary mt-0.5">{result.traits.pronunciation.toFixed(1)}</p>
              </div>
              <div className="p-3 rounded-xl bg-surface-2/40 border border-border/60 text-center">
                <p className="text-[10px] font-bold uppercase text-muted-foreground">Rhythm & Pace</p>
                <p className="text-base font-black text-primary mt-0.5">{result.traits.rhythm_and_pace.toFixed(1)}</p>
              </div>
              <div className="p-3 rounded-xl bg-surface-2/40 border border-border/60 text-center">
                <p className="text-[10px] font-bold uppercase text-muted-foreground">Intelligibility</p>
                <p className="text-base font-black text-primary mt-0.5">{result.traits.intelligibility.toFixed(1)}</p>
              </div>
              <div className="p-3 rounded-xl bg-surface-2/40 border border-border/60 text-center">
                <p className="text-[10px] font-bold uppercase text-muted-foreground">Repetition Match</p>
                <p className="text-base font-black text-primary mt-0.5">{result.traits.repetition_accuracy.toFixed(1)}</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-1.5 p-3 rounded-xl bg-background/80 border border-border/80 text-sm leading-loose">
                {result.wordDiff.tokens.map((tok, tIdx) => {
                  if (tok.type === 'correct') {
                    return (
                      <span key={tIdx} className="rounded bg-emerald-500/15 text-emerald-400 px-1.5 py-0.5 font-medium">
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center gap-2">
            {currentItem.focusSkills.map((skill) => (
              <span key={skill} className="inline-flex items-center gap-1 text-[11px] text-muted-foreground bg-surface-2 px-2 py-0.5 rounded-md">
