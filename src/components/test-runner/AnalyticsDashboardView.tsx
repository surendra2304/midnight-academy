    ...
    return (
      <div className="rounded-2xl border border-border bg-card/40 p-12 text-center space-y-4">
        ...
        <p className="mx-auto max-w-md text-xs text-muted-foreground">
          Complete your first practice question or full-length mock test to unlock personalized
          skill analytics, error patterns, and weakness diagnostics.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-xl border border-primary/40 bg-primary/5 p-5">
          ...
          <p className="text-3xl font-black text-primary mt-1">
            {profile.latestOverallBand.toFixed(1)}{" "}
            <span className="text-xs font-semibold text-muted-foreground">/ 6.0</span>
          </p>
          <p className="text-[11px] text-muted-foreground mt-1">
            Best Score: <strong>{profile.bestOverallBand.toFixed(1)}</strong>
          </p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-surface-2/40 p-5">
          ...
          <p className="text-3xl font-black text-foreground mt-1">
            {profile.averageOverallBand.toFixed(1)}{" "}
            <span className="text-xs font-semibold text-muted-foreground">/ 6.0</span>
          </p>
          <p className="text-[11px] text-muted-foreground mt-1">
            {profile.totalTestsCompleted} Tests Completed
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-6 space-y-4">
          ...
          <div className="space-y-3">
            {profile.topWeakSkills.map((skill) => (
              <div
                key={skill.skillName}
                className="rounded-xl border border-border/80 bg-background/60 p-4 space-y-2"
              >
                ...
                <div className="h-1.5 w-full rounded-full bg-surface-2 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${skill.accuracyPercent}%` }}
                  />
                </div>
              </div>
            ))}
        ...
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div>
            ...
            <h3 className="text-base font-bold text-foreground">
              Section Performance vs Platform Learners
            </h3>
          </div>
          <span className="text-[11px] text-muted-foreground">
            Aggregated across 50+ cohort attempts
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {(["reading", "listening", "writing", "speaking"] as const).map((sec) => {
            ...
            return (
              <div
                key={sec}
                className="rounded-xl border border-border/80 bg-surface-2/30 p-4 space-y-2"
              >
                <div className="flex items-center justify-between">
                ...
                <p className="text-xs text-muted-foreground">
                  Your accuracy exceeds <strong className="text-foreground">{percentile}%</strong>{" "}
                  of platform learners.
                </p>
      ...
      <section className="rounded-2xl border border-primary/30 bg-card/60 p-6 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-foreground">
              AI Diagnostic Insight & Study Strategy
            </h3>
          </div>
          {!aiExplanation ? (
        ...
        {aiExplanation ? (
          ...
          <p className="text-xs text-muted-foreground">
            Generate an AI learning advisor analysis based on your deterministic error trends and
            target score gap.
          </p>
        )}
      </section>
            ...
            <tbody className="divide-y divide-border/60">
              {profile.taskTypeBreakdown.map((t) => (
                <tr key={t.itemType} className="hover:bg-surface-2/30 transition-colors">
                  ...
                  <td className="py-3">
                    <span
                      className={`font-bold ${t.accuracyPercent >= 70 ? "text-success" : t.accuracyPercent >= 50 ? "text-warning" : "text-destructive"}`}
                    >
                      {t.accuracyPercent}%
                    </span>
                  </td>
