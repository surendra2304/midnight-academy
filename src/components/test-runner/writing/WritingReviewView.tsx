  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-xl border border-border bg-card/60 p-6">
        <span className="text-xs font-bold uppercase tracking-wider text-primary">Writing Section Feedback</span>
        <h2 className="text-xl font-extrabold text-foreground mt-1">AI Rubric Evaluation & Corrections</h2>
        <p className="text-xs text-muted-foreground mt-1">
          Review your 1.0–6.0 score bands, trait breakdown, grammatical corrections, and model response.
        </p>
      </section>

      <div className="space-y-8">
        {reviews.map((rev, idx) => {
          const { item, studentResponse, evaluation, modelAnswer, isDeterministic, deterministicScore, deterministicFeedback } = rev;
            ...
            <article key={item.id} className="rounded-xl border border-border bg-card/40 p-6 space-y-6">
                ...
                {isDeterministic ? (
                  <div className="flex items-center gap-2 rounded-lg border border-primary/40 bg-primary/10 px-4 py-2">
                    ...
                    <div>
                      ...
                      <p className="text-base font-black text-primary">{deterministicScore === 1.0 ? '100%' : '0%'}</p>
                    </div>
                  </div>
                ) : evaluation ? (
                  <div className="flex items-center gap-2 rounded-lg border border-primary/40 bg-primary/10 px-4 py-2">
                    <Award className="size-5 text-primary" />
                    <div>
                      <p className="text-xs text-muted-foreground font-semibold">TOEFL Band Score</p>
              ...
              {evaluation?.traits ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="rounded-lg border border-border bg-surface-2/30 p-3 text-center">
                    <p className="text-[11px] font-semibold text-muted-foreground uppercase">Task Fulfillment</p>
                    <p className="text-lg font-extrabold text-foreground">{evaluation.traits.task_fulfillment?.toFixed(1) || '—'}</p>
                  </div>
                  <div className="rounded-lg border border-border bg-surface-2/30 p-3 text-center">
                    <p className="text-[11px] font-semibold text-muted-foreground uppercase">Organization</p>
                    <p className="text-lg font-extrabold text-foreground">{evaluation.traits.organization?.toFixed(1) || '—'}</p>
                  </div>
                  <div className="rounded-lg border border-border bg-surface-2/30 p-3 text-center">
                    <p className="text-[11px] font-semibold text-muted-foreground uppercase">Language Use</p>
                    <p className="text-lg font-extrabold text-foreground">{evaluation.traits.language_use?.toFixed(1) || '—'}</p>
                  </div>
                </div>
              ) : null}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs leading-relaxed">
                ...
                <div className="rounded-lg border border-border bg-background/50 p-4 space-y-2">
                  ...
                  <p className="text-foreground/90 whitespace-pre-line">{studentResponse || '(No response submitted)'}</p>
                  {deterministicFeedback ? (
                    <p className="text-muted-foreground italic border-t border-border/50 pt-2">{deterministicFeedback}</p>
                  ) : null}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {evaluation.corrections.map((corr, cIdx) => (
                      <div key={cIdx} className="rounded-lg border border-border/80 bg-background/60 p-3 text-xs space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="line-through text-destructive font-medium">{corr.original}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  {evaluation?.strengths && evaluation.strengths.length > 0 ? (
                    <div className="rounded-lg border border-success/30 bg-success/5 p-4 space-y-1.5">
                      <span className="font-bold text-success uppercase text-[10px]">Strengths:</span>
                  ...
                  {evaluation?.next_actions && evaluation.next_actions.length > 0 ? (
                    <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-1.5">
                      <span className="font-bold text-primary uppercase text-[10px]">Recommended Practice:</span>
                      <ul className="list-disc list-inside space-y-1 text-foreground/90">
